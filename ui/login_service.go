...
func LoginService(m Model) (tea.Model, tea.Cmd) {
    ...
    if user == "" || passwd == "" {
        log.Println("Error: Campos vacíos")
        return m, nil
    }

    log.Printf("Intentando login para usuario: %s\n", user)

    resp, _, err := api.DoRequest(fmt.Sprintf("/session/login?username=%s&password=%s", user, passwd))
    if err != nil {
        log.Printf("Error de red/API: %v\n", err)
        return m, nil
    }

    loginSuccess, ok := resp["login"].(bool)
    if !ok || !loginSuccess {
        log.Println("Login rechazado: credenciales incorrectas")
        return m, nil
    }

    log.Println("Login exitoso, cambiando a perfil")
